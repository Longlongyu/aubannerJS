 ...
 (function(window) {
  ...
  function init(elem, options) {
    ...
    this.animat = function(anim, num) {
      ...
      if (self.options.anim && self.options.anim == 'none') {
        clearChildrenNodes(self.curr.parentNode, self.curr)
        setNavSelect(self.curr.getAttribute('aubanner-img'));
        return;
      }
      if (anim == 'left') { // 左滑动
        resetTrans(25);
        ...
        setTimeout(function() {
          ...
          setNavSelect(self.curr.getAttribute('aubanner-img'));
          self.banner.style.left =  -(self.curr.width * num) + 'px';
        }, 25);
      ...
      this.btns.forEach(function(e) {
        ...
        e.addEventListener('click', function navclick(event) {
          var ele = event.target,
              ...
              selectpos = parseInt(self.select.getAttribute('aubanner-btn'));
          if (ele === self.select) return;
          var result = 'none';
          ...
          self.animat(result, Math.abs(selectpos - pos));
    ...
    { // 初始化left&&right按钮
      ...
      .forEach(function(e) {
        if (self.pre.getAttribute('aubanner-pre') === 'normal') {
          e.style.width = self.imgs[0].width / 6 + 'px';
          e.style.height = self.imgs[0].height + 'px';
          e.style.display = 'inline-block';
